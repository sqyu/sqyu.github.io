...
def standardconf():
  ...
  <html lang="HTMLLANG" xml:lang="HTMLLANG" xmlns="http://www.w3.org/1999/xhtml">
  <head>
...
def procfile(f):
  ...
  while pc(f, False) == '#':
	...
	elif b.startswith('lang-'):###
	  html_lang = b.replace("lang-", "")###
  ...
  out(f.outf, f.conf['firstbit'].replace("HTMLLANG", html_lang))
  ...
  if showfooter and (showlastupdated or showsourcelink):
    ...
    if showlastupdated:
      ...
      update_texts = {"en":"Last updated", "zh-Hant":"最後修訂", "zh-Hans":"最后更新", "ja":"最終更新日時", "ko":"최종 업데이트 일시", "fr":"Dernière modification"}
      hb(f.outf, f.conf['lastupdated'].replace("Page generated", update_texts[html_lang]), s)
    if showsourcelink:
...
def main():
  ...
  if sys.argv[1] == '--show-config':
    print standardconf().replace("HTMLLANG", "en")
    raise SystemExit
